...
import {
  ...
  Globe, Check, Sparkles, Send, ExternalLink, AlertCircle, User2, Building2,
  KeyRound, CreditCard, Bot, Radio,
} from "lucide-react";
...
function WhatsAppIcon() {
  return (
    <svg viewBox="0 0 48 48" className="h-5 w-5" fill="none">
      ...
      <path d="M34.5 13.5A14.47 14.47 0 0 0 24 9C16.27 9 10 15.27 10 23c0 2.5.66 4.95 1.9 7.1L9 38l8.1-2.9A14.95 14.95 0 0 0 24 37c7.73 0 14-6.27 14-14a13.95 13.95 0 0 0-3.5-9.5ZM24 35a12.94 12.94 0 0 1-6.6-1.8l-.47-.28-4.87 1.74 1.23-4.74-.3-.49A13 13 0 1 1 24 35Zm7.11-9.74c-.39-.2-2.3-1.14-2.66-1.27-.36-.13-.62-.2-.88.2s-1.01 1.27-1.24 1.53c-.23.26-.46.3-.85.1a10.67 10.67 0 0 1-3.14-1.94 11.75 11.75 0 0 1-2.17-2.7c-.23-.4-.02-.61.17-.81.18-.18.4-.46.6-.69.2-.23.26-.4.4-.66.13-.26.06-.49-.03-.69-.1-.2-.88-2.12-1.2-2.9-.32-.77-.64-.66-.88-.67h-.75c-.26 0-.68.1-1.04.49s-1.36 1.33-1.36 3.25 1.4 3.77 1.59 4.03c.2.26 2.74 4.18 6.64 5.86.93.4 1.65.64 2.22.82.93.3 1.78.26 2.45.16.75-.12 2.3-.94 2.63-1.85.32-.91.32-1.69.22-1.85-.1-.16-.36-.26-.75-.46Z" fill="white"/>
    </svg>
  );
}
...
function MessengerIcon() {
  return (
    <svg viewBox="0 0 48 48" className="h-5 w-5" fill="none">
      ...
      <path d="M24 9C15.16 9 8 15.73 8 24.07c0 4.46 1.82 8.4 4.77 11.17.25.22.4.54.41.87l.08 2.74a1.06 1.06 0 0 0 1.5.94l3.05-1.35c.26-.11.55-.14.82-.07A17.5 17.5 0 0 0 24 39c8.84 0 16-6.73 16-15.07C40 15.58 32.84 9 24 9Zm9.64 11.52-4.8 7.63a2.5 2.5 0 0 1-3.63.67l-3.82-2.86a1 1 0 0 0-1.21 0l-5.15 3.91c-.69.52-1.58-.32-1.1-1.06l4.8-7.63a2.5 2.5 0 0 1 3.63-.67l3.82 2.86a1 1 0 0 0 1.21 0l5.15-3.91c.69-.52 1.58.32 1.1 1.06Z" fill="white"/>
    </svg>
  );
}
...
function TikTokIcon() {
  return (
    <svg viewBox="0 0 48 48" className="h-5 w-5" fill="none">
      ...
      <path d="M34 19.42a8.98 8.98 0 0 1-5.25-1.67v7.62a6.96 6.96 0 1 1-6.96-6.96c.25 0 .5.01.74.04v3.85a3.17 3.17 0 1 0 2.22 3.07V10h3.84a5.26 5.26 0 0 0 5.41 5.41v4Z" fill="#25F4EE"/>
      <path d="M34 19.42a8.98 8.98 0 0 1-5.25-1.67v7.62a6.96 6.96 0 1 1-6.96-6.96c.25 0 .5.01.74.04v3.85a3.17 3.17 0 1 0 2.22 3.07V10h3.84a5.26 5.26 0 0 0 5.41 5.41v4Z" fill="white" opacity="0.4"/>
      <path d="M33 18.42a8.98 8.98 0 0 1-5.25-1.67v7.62a6.96 6.96 0 1 1-6.96-6.96c.25 0 .5.01.74.04v3.85a3.17 3.17 0 1 0 2.22 3.07V9h3.84a5.26 5.26 0 0 0 5.41 5.41v4Z" fill="#FE2C55"/>
    </svg>
  );
}
...
function Settings() {
  ...
  const saveWhatsAppMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t("WhatsApp Business") + " " + t("Profile saved")); queryClient.invalidateQueries({ queryKey: ["settings"] }); setWaToken(""); },
  ...
  const saveMessengerMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t("Facebook Messenger") + " " + t("Profile saved")); queryClient.invalidateQueries({ queryKey: ["settings"] }); setMsgToken(""); },
    onError: (e: Error) => toast.error(e.message ?? t("Save failed")),
  });
  ...
  const saveInstagramMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Instagram " + t("Profile saved")); queryClient.invalidateQueries({ queryKey: ["settings"] }); setIgToken(""); },
    onError: (e: Error) => toast.error(e.message ?? t("Save failed")),
  });
  ...
  const saveTikTokMutation = useMutation({
    mutationFn: () => api.settings.saveTikTok({ appId: ttAppId, accountId: ttAccountId, appSecret: ttAppSecret, accessToken: ttAccToken, verifyToken: ttVerify }),
    onSuccess: () => { toast.success(t("TikTok for Business") + " " + t("Profile saved")); queryClient.invalidateQueries({ queryKey: ["settings"] }); setTtAppSecret(""); setTtAccToken(""); },
    onError: (e: Error) => toast.error(e.message ?? t("Save failed")),
  });
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <Tabs defaultValue="profile" className="w-full">
        <TabsList className="h-auto p-1 flex flex-wrap gap-1 bg-muted/60 rounded-xl mb-6">
          <TabsTrigger value="profile" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <User2 className="h-3.5 w-3.5" />{t("Profile")}
          </TabsTrigger>
          <TabsTrigger value="apikeys" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <KeyRound className="h-3.5 w-3.5" />{t("API Keys")}
          </TabsTrigger>
          <TabsTrigger value="persona" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <Bot className="h-3.5 w-3.5" />{t("AI Persona")}
          </TabsTrigger>
          <TabsTrigger value="channels" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <Radio className="h-3.5 w-3.5" />{t("Channels")}
          </TabsTrigger>
          <TabsTrigger value="billing" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <CreditCard className="h-3.5 w-3.5" />{t("Billing")}
          </TabsTrigger>
          <TabsTrigger value="account" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <Building2 className="h-3.5 w-3.5" />{t("Account")}
          </TabsTrigger>
          <TabsTrigger value="widget" className="flex items-center gap-1.5 rounded-lg text-xs px-3 py-2 data-[state=active]:shadow-sm">
            <Globe className="h-3.5 w-3.5" />{t("Web Widget")}
          </TabsTrigger>
        </TabsList>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <Button
                    ...
                    disabled={saveProfileMutation.isPending || Object.keys(profileDraft).length === 0}
                    className="mt-2"
                  >
                    {saveProfileMutation.isPending && <Loader2 className="h-4 w-4 animate-spin me-2" />}
                    {t("Save changes")}
                  </Button>
                </>
              )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={saveKeyMutation.isPending || !newKey.startsWith("sk-") && !newKey.startsWith("AIza")}
                      >
                        {saveKeyMutation.isPending && <Loader2 className="h-4 w-4 animate-spin me-2" />}
                        {t("Save")}
                      </Button>
                    </div>
                  </div>
              ...
              {isLoading ? (
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{t("This text will be injected into the AI's system instructions.")}</p>
                    <textarea
                      value={systemPrompt}
                  ...
                  >
                    {savePersonaMutation.isPending && <Loader2 className="h-4 w-4 animate-spin me-2" />}
                    {t("Save persona")}
                  </Button>
                </>
              )}
          ...
          <div className="rounded-xl border border-border bg-muted/30 px-5 py-4 flex items-start gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                {t("Connect your agency AI to WhatsApp, Messenger, Instagram, and TikTok using your own API keys. Each channel needs a webhook URL configured in the respective developer portal.")}
              </div>
            </div>
          </div>
            ...
            <>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-xs">{t("Meta Cloud API · Bring your own token")}</CardDescription>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input value={waPhoneId} onChange={e => setWaPhoneId(e.target.value)} placeholder="12345678901234" dir="ltr" className="text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("Business Account ID")}</Label>
                      <Input value={waBusinessId} onChange={e => setWaBusinessId(e.target.value)} placeholder="98765432109876" dir="ltr" className="text-sm" />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      {t("Permanent Access Token")} {ch?.whatsapp?.connected && <span className="text-muted-foreground">({t("Leave blank to keep current")})</span>}
                    </Label>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <a href="https://developers.facebook.com/apps" target="_blank" rel="noopener noreferrer" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1 transition-colors">
                    ...
                    <Button size="sm" onClick={() => saveWhatsAppMutation.mutate()} disabled={saveWhatsAppMutation.isPending}>
                      {saveWhatsAppMutation.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin me-1.5" />}
                      {t("Save WhatsApp")}
                    </Button>
                  </div>
                </CardContent>
              ...
              <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-xs">{t("Meta Graph API · Bring your own Page token")}</CardDescription>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      {t("Page Access Token")} {ch?.messenger?.connected && <span className="text-muted-foreground">({t("Leave blank to keep current")})</span>}
                    </Label>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <Button size="sm" onClick={() => saveMessengerMutation.mutate()} disabled={saveMessengerMutation.isPending}>
                      {saveMessengerMutation.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin me-1.5" />}
                      {t("Save Messenger")}
                    </Button>
                  </div>
                </CardContent>
              ...
              <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-xs">{t("Meta Graph API · Instagram Business / Creator")}</CardDescription>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      {t("Instagram Graph API Token")} {ch?.instagram?.connected && <span className="text-muted-foreground">({t("Leave blank to keep current")})</span>}
                    </Label>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <Button size="sm" onClick={() => saveInstagramMutation.mutate()} disabled={saveInstagramMutation.isPending}>
                      {saveInstagramMutation.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin me-1.5" />}
                      {t("Save Instagram")}
                    </Button>
                  </div>
                </CardContent>
              ...
              <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-xs">{t("TikTok Business API · Direct Messaging")}</CardDescription>
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      {t("App Secret")} {ch?.tiktok?.connected && <span className="text-muted-foreground">({t("Leave blank to keep current")})</span>}
                    </Label>
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      {t("Access Token")} {ch?.tiktok?.connected && <span className="text-muted-foreground">({t("Leave blank to keep current")})</span>}
                    </Label>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <a href="https://developers.tiktok.com" target="_blank" rel="noopener noreferrer" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1 transition-colors">
                    ...
                    <Button size="sm" onClick={() => saveTikTokMutation.mutate()} disabled={saveTikTokMutation.isPending}>
                      {saveTikTokMutation.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin me-1.5" />}
                      {t("Save TikTok")}
                    </Button>
                  </div>
                </CardContent>
        ...
        <TabsContent value="billing">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {subscription ? `${subscription.plan} ${t("plan")} · ${t("renews")} ${subscription.renewsAt}` : "—"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {isLoading ? (
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-3 gap-4">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div key={i}><Skeleton className="h-4 w-1/2 mb-2" /><Skeleton className="h-6 w-3/4" /></div>
                    ))}
                  </div>
                ...
                <>
                  <div className="grid sm:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-2xl font-bold">{subscription?.creditsLeft?.toLocaleString() ?? "—"}</div>
                    </div>
                    <div>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <div className="flex gap-2 pt-1">
                    <Button
                      variant="outline"
                      onClick={() => { navigator.clipboard.writeText(settings?.user?.email ?? ""); toast(t("Copied")); }}
                    >
                      <Copy className="h-4 w-4 me-2" />{t("Copy email")}
                    </Button>
                  </div>
                </>
              )}
                ...
                <CardContent className="space-y-4">
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            ...
                            boxShadow: wColor === p.value ? `0 0 0 2px white, 0 0 0 4px ${p.value}` : "none",
                          }}
                        >
                  ...
                  <div className="pt-2">
                    <Button onClick={() => saveWidgetMutation.mutate()} disabled={saveWidgetMutation.isPending} className="w-full">
                      {saveWidgetMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin me-2" /> : null}
        ...
        <div className="flex items-center gap-2.5 px-3.5 py-2.5" style={{ background: grad }}>
          ...
          <div>
            ...
            <div className="text-[9px] text-white/70 mt-0.5 flex items-center gap-1">
              <span className="h-1 w-1 rounded-full bg-emerald-400 inline-block animate-pulse" /> Online
            </div>
          </div>
        </div>
        {/* Chat */}
